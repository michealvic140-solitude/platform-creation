...
-- Virtual Gangs: audit trail, settlement tracking, configurable rewards, atomic single bet placement

-- 1. Tracking columns on matches
...
-- 2. Configurable virtual reward / limits
...
-- 3. Seed default Gang A / Gang B teams
...
-- 5. Wrap resolve to add audit log + settled_by + configurable bonuses
...
BEGIN
  ...
  -- Credit winners (payout * multiplier + win bonus)
  WITH won_bets AS (
...
-- 6. Single-selection instant bet placement for virtuals (atomic w/ balance check)
